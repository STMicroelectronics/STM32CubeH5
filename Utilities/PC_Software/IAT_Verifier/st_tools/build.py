...
def args():
    ...
    asm = subs.add_parser('asm', help='build boot')
    asm.add_argument("infile", help="input filename" )
    asm.add_argument("-f", '--file', metavar='filename', required=True)
    asm.add_argument("outfile", help="output filename" )
    asm.add_argument("-i", "--input-size", type=intparse, required=True)
    asm.add_argument("-o", "--output-size", type=intparse, required=True)
