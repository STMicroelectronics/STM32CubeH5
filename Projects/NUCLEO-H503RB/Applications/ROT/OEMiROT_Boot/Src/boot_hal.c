...
void boot_platform_noimage(void)
{
  ...
  nsslib_jump_HDP_lvl2 = (uint32_t)(NSSLIB_PFUNC->JumpHDPLvl2);
  ...
  boot_jump_to_NSS((uint32_t)&boot_jump_to_NSS, nsslib_jump_HDP_lvl2, (uint32_t) BOOTLOADER_BASE, 7U);
...
/**
  ...
  */
__attribute__((naked)) void boot_jump_to_NSS(uint32_t boot_jump_addr1, uint32_t boot_jump_addr2,
                                             uint32_t vector_table, uint32_t MPU_region)
{
  __ASM volatile(
#if !defined(__ICCARM__)
...
void boot_platform_quit(struct boot_arm_vector_table *vector)
{
   ...
   (void)fih_delay();
    ...
    /* Init NSS jump function descriptor */
    nsslib_jump_HDP_lvl3 = (uint32_t)(NSSLIB_PFUNC->JumpHDPLvl3);
    ...
    boot_jump_to_NSS((uint32_t)&boot_jump_to_NSS, nsslib_jump_HDP_lvl3, (uint32_t) vt, 1U);
    /* Avoid compiler to pop registers after having changed MSP */
#if !defined(__ICCARM__)
    __builtin_unreachable();
#else
...
void boot_erase_image(void)
{
  ...
  {
    if (FLASH_DEV_NAME.EraseSector(sector) != ARM_DRIVER_OK)
    {
      Error_Handler();
    }
  }
  ...
  /* Erase data image */
  for (sector = FLASH_AREA_5_OFFSET;
       sector < (FLASH_AREA_5_OFFSET + FLASH_AREA_5_SIZE);
       sector += FLASH_AREA_IMAGE_SECTOR_SIZE)
  {
    if (FLASH_DEV_NAME.EraseSector(sector) != ARM_DRIVER_OK)
    {
