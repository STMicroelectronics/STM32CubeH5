...
/**
  ...
  *          using the STM32H5xx WWDG LL API.
...
int main(void)
{
  ...
  while (1)
  {
    if (1 != KeyPressed)
    {
      /* Refresh WWDG Downcounter to initial value around 132ms */
...
static void MX_WWDG_Init(void)
{
  ...
  /* Activate WWDG */
  /* set prescaler to have a rollover each about 132ms */
  /* set window value to same value (around 132ms) as downcounter in order to be able to refresh the WWDG almost immediately */
