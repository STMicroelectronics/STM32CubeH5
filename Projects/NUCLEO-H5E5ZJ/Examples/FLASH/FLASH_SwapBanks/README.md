...
1- Choose the FLASH_SwapBanks_Bank2 project and build it using your preferred toolchain (EWARM, MDK-ARM, STM32CubeIDE),then use STM32CubeProgrammer to flash the 
  generated binary file FLASH_BANK2.bin into the bank2 at the address 0x08200000.
...
- If program is executing from bank1 is selected, LED1(GREEN) toggles,

- If program is executing from bank2 is selected, LED3(BLUE) toggles,

- If error occurred, LED2(RED) ON
...
### <b> Directory contents </b>
  ...
  - FLASH/FLASH_SwapBanks/Src/stm32h5xx_hal_msp.c         HAL MSP module 
...
<b>EWARM</b>
  ...
  - Load the binary file generated with FLASH_SwapBanks_Bank2 into
    the BANK2 at address 0x08200000 using STM32CubeProgrammer.
...
<b>MDK-ARM</b>
  ...
  - Rebuild all files.
  - Load the binary file generated with FLASH_SwapBanks_Bank1 into
    the BANK1 at address 0x08000000 using STM32CubeProgrammer.
  - Load the binary file generated with FLASH_SwapBanks_Bank2 into
    the BANK2 at address 0x08200000 using STM32CubeProgrammer.
  - Choose the FLASH_SwapBanks_Bank1 project and run the example.
...
To actually load one of the binaries into 0x8200000, you have to set up the debug configuration to load one bin file with an offset of 0x200000:

- Select **Debug configuration** or **Run configuration** depending on the active build (FLASH_SwapBanks_Bank1 configuration).
- Double-click on **STM32 Cortex-M C/C++ Application**.
- Select **Startup** > **Add**.
- Select the **FLASH_SwapBanks_Bank2** project.
- Uncheck the **Perform build** option.
- Add `0x200000` to **Use download offset hex**.
- Click **Debug/Run** to debug or run the example.
