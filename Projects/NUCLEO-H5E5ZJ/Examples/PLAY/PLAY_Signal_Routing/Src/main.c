...
int main(void)
{
  ...
  while (1)
  {
    ...
    tim_oc_config.OCMode = TIM_OCMODE_FORCED_ACTIVE;

    if (HAL_TIM_OC_ConfigChannel(&htim2, &tim_oc_config, TIM_CHANNEL_1) != HAL_OK)
    {
      Error_Handler();
    }
    ...
    tim_oc_config.OCMode = TIM_OCMODE_FORCED_INACTIVE;
...
static void MX_PLAY1_Init(void)
{
  ...
  */
  ...
  configINPUTs[0].source = HAL_PLAY1_IN_TIM2_TRGO_MUX0;
  ...
  */
  ...
  configLUTs[0].truth_table = 0xAAAA;
  configLUTs[0].input_source[HAL_PLAY_LUT_INPUT0] = HAL_PLAY_LUT_INPUT_FILTER0;
