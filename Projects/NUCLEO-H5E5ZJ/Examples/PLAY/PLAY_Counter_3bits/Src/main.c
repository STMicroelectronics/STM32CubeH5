...
#define CONFIGURED_LUT_NBR     3U
#define CONFIGURED_OUTPUT_NBR  3U
...
/* USER CODE BEGIN PFP */
static void lut_output_falling_callback(HAL_PLAY_HandleTypeDef *hplay, uint32_t edge_trig_mask_status);
/* USER CODE END PFP */
...
static void MX_PLAY1_Init(void)
{
  ...
  */
  ...
  configLUTs[0].input_source[HAL_PLAY_LUT_INPUT1] = HAL_PLAY_LUT_INPUT_DEFAULT;
  ...
  configLUTs[0].clk_gate_source = HAL_PLAY_LUT_CLK_GATE_FILTER12;
  ...
  */
  ...
  configLUTs[1].clk_gate_source = HAL_PLAY_LUT_CLK_GATE_FILTER12;
  ...
  */
  ...
  configLUTs[2].clk_gate_source = HAL_PLAY_LUT_CLK_GATE_FILTER12;
  ...
  */
  ...
  configOUTPUTs[0].lut_output = HAL_PLAY_LUT0_OUT_REGISTERED;

  /** Configure Output 13
  */
  configOUTPUTs[1].output_mux = HAL_PLAY_OUT13;
  configOUTPUTs[1].lut_output = HAL_PLAY_LUT1_OUT_REGISTERED;

  /** Configure Output 14
  */
  configOUTPUTs[2].output_mux = HAL_PLAY_OUT14;
  configOUTPUTs[2].lut_output = HAL_PLAY_LUT2_OUT_REGISTERED;
...
/** Callback for LUT Output Falling Edge
  * This function is called when the LUT2 output signal falls (i.e., when the counter resets to 0).
  */
static void lut_output_falling_callback(HAL_PLAY_HandleTypeDef *hplay, uint32_t edge_trig_mask_status)
{
  ...
  if ((edge_trig_mask_status & HAL_PLAY_LUT2_OUT_REGISTERED) == HAL_PLAY_LUT2_OUT_REGISTERED)
  {
    /** Toggle the LED time the counter resets to 0
      * (falling edge of most significant bit signal).
      */
