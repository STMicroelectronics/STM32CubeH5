...
int main(void)
{
  ...
  /* in SystemInit() based on partition_stm32h5e5xx.h file's definitions. */
...
static void SystemIsolation_Config(void)
{
  ...
  /* of 512 bytes.                                                            */
  /* Internal SRAM3 (starting from 0x20060000) will be configured as          */
  /* fully non-secure.                                                        */
  ...
  /* Release PA.03 I/O for LED_GREEN control on non-secure                */
   __HAL_RCC_GPIOA_CLK_ENABLE();
   HAL_GPIO_ConfigPinAttributes(GPIOA, GPIO_PIN_3, GPIO_PIN_NSEC);
