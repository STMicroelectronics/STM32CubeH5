...
ld_tz_s_file="$appli_dir/STM32CubeIDE/Secure/STM32H5F5LJHXQ_FLASH.ld"
ld_tz_ns_file="$appli_dir/STM32CubeIDE/NonSecure/STM32H5F5LJHXQ_FLASH.ld"

# CubeIDE project Appli Full Secure files
cube_appli_postbuild="$appli_dir/STM32CubeIDE/postbuild.sh"
ld_file="$appli_dir/STM32CubeIDE/STM32H5F5LJHXQ_FLASH.ld"
...
if [ "$Full_secure" == "1" ]; then
  ...
  echo "stm32h5f5xx_flash.icf and main.h successfully updated according to STiRoT_Config.obk"
  if [ "$script_mode" != "AUTO" ]; then $SHELL; fi
