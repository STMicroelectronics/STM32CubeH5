...
set remove_protect_init=-ob SECWM1_STRT=1 SECWM1_END=0 WRPSG11=0xffffffff WRPSG22=0xffffffff SECWM2_STRT=1 SECWM2_END=0 HDP1_STRT=1 HDP1_END=0 HDP2_STRT=1 HDP2_END=0 SECBOOT_LOCK=0xC3 SWAP_BANK=0 SRAM2_RST=0 SRAM2_ECC=0 BOOT_UBE=0xB4
...
set write_protect=WRPSG11=%wrpgrp1% WRPSG22=%wrpgrp2%
...
echo %action%
%stm32programmercli% %connect_no_reset% -d %cube_fw_path%\Projects\STM32H5F5J-DK\%oemirot_boot_path_project%\Binary\OEMiROT_Boot.bin %bootaddress% -v
IF !errorlevel! NEQ 0 goto :error
