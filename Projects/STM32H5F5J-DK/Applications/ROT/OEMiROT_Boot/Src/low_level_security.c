...
static void tamper_loader_cfg(void);;
#endif
#if defined(MCUBOOT_EXT_LOADER)
...
static void nvic_loader_cfg(void)
{
  ...
  else
  {
    ...
    uint32_t itns4 = NVIC->ITNS[4U];
    uint32_t itns5 = NVIC->ITNS[5U];
    ...
    if ((itns0 != RSS_NVIC_INIT_ITNS0_VAL) ||
        ...
        (itns4 != RSS_NVIC_INIT_ITNS4_VAL) ||
        (itns5 != RSS_NVIC_INIT_ITNS5_VAL) ||
...
static void tamper_loader_cfg(void)
{
  ...
  if (uFlowStage == FLOW_STAGE_CFG)
  {
    ...
    FLOW_CONTROL_STEP(uFlowProtectValue, FLOW_STEP_TAMP_L_EN, FLOW_CTRL_TAMP_L_EN);
  }
  /* verification stage */
  else
  {
    uint32_t tamp = TAMP->SECCFGR;

    if (tamp != TAMP_INIT_SECCFGR_VAL)
    {
      Error_Handler();
    }
    else
    {
      FLOW_CONTROL_STEP(uFlowProtectValue, FLOW_STEP_TAMP_L_CH, FLOW_CTRL_TAMP_L_CH);
    }
  }
}
