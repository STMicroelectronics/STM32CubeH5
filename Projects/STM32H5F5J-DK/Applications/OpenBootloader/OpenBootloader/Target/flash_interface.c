...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define FLASH_SECTOR_NUMBER              ((uint16_t)512U)
...
static void OPENBL_FLASH_Program(uint32_t flash_address, uint32_t data_address);
static ErrorStatus OPENBL_FLASH_EnableWriteProtection(uint8_t *p_list_of_sectors, uint32_t length);
...
ErrorStatus OPENBL_FLASH_SetWriteProtection(FunctionalState state, uint8_t *p_list_of_sectors, uint32_t length)
{
  ...
  if (state == ENABLE)
  {
    OPENBL_FLASH_EnableWriteProtection(p_list_of_sectors, length);
...
/**
  ...
  * @param  flash_address specifies the address to be programmed.
  * @param  data_address specifies the address of data to be programmed.
  * @retval None.
  */
static void OPENBL_FLASH_Program(uint32_t flash_address, uint32_t data_address)
{
  ...
  HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, flash_address, data_address);
}
...
static ErrorStatus OPENBL_FLASH_EnableWriteProtection(uint8_t *p_list_of_sectors, uint32_t length)
{
  ...
  {
    if (*(p_list_of_sectors + index) < FLASH_SECTOR_127)
    {
      flash_ob.WRPSector |= (uint32_t)1U << (*(p_list_of_sectors + index));
  ...
  for (index = 0U; index < length; index++)
  {
    if (*(p_list_of_sectors + index) >= FLASH_SECTOR_127)
    {
      flash_ob.WRPSector |= (uint32_t)1U << ((*(p_list_of_sectors + index)) - (FLASH_SECTOR_127 + 1U));
...
#endif /* (__ICCARM__) */
{
  ...
  while (__HAL_FLASH_GET_FLAG(FLASH_FLAG_BSY))
  {
    ...
    {
      /* NOTE: Nothing to do as I2C is not present in this application */
    }
...
/**
  ...
  * @param[out]  p_page_error pointer to variable that contains the configuration
...
#endif /* (__ICCARM__) */
{
  ...
  if (status == HAL_OK)
  {
    /* Initialization of p_page_error variable */
    *p_page_error = 0xFFFFFFFFU;
    ...
    if (((p_erase_init->Banks) & FLASH_BANK_1) == FLASH_BANK_1)
    {
      /* Reset Sector Number for Bank1 */
      CLEAR_BIT((*reg_cr), (FLASH_CR_BKSEL | FLASH_CR_SNB));
    }

    if (((p_erase_init->Banks) & FLASH_BANK_2) == FLASH_BANK_2)
    {
      /* Reset Sector Number for Bank2 */
      CLEAR_BIT((*reg_cr), FLASH_CR_SNB);
      SET_BIT((*reg_cr), FLASH_CR_BKSEL);

      p_erase_init->Sector = p_erase_init->Sector - (FLASH_SECTOR_255 + 1U);
    }

    /* Proceed to erase the sector */
    SET_BIT((*reg_cr), (FLASH_CR_SER | (p_erase_init->Sector << FLASH_CR_SNB_Pos) | FLASH_CR_START));
    ...
    if (status != HAL_OK)
    {
      ...
      *p_page_error = p_erase_init->Sector;
    }
  }
