...
static uint32_t GetSector(uint32_t uAddr)
{
  ...
  if((uAddr >= FLASH_BASE) && (uAddr < FLASH_BASE + FLASH_BANK_SIZE))
  {
    sector = (uAddr & ~FLASH_BASE) / FLASH_SECTOR_SIZE;
  }
  else if ((uAddr >= FLASH_BASE + FLASH_BANK_SIZE) && (uAddr < FLASH_BASE + FLASH_SIZE))
  {
    sector = ((uAddr & ~FLASH_BASE) - FLASH_BANK_SIZE) / FLASH_SECTOR_SIZE;
  }
  else
  {
    sector = 0xFFFFFFFF; /* uAddr out of range */
  }
...
static HAL_StatusTypeDef FLASH_If_Erase_Size(void *pStart, uint32_t uLength)
{
  ...
  uint32_t nb_sector = 0U;
  uint32_t chunk_nb_sector = 0U;
  ...
  if (e_ret_status == HAL_OK)
  {
    ...
    if (HAL_FLASH_Unlock() == HAL_OK)
    {
      ...
      nb_sector = GetSector(uStart + uLength - 1U) - first_sector + 1U;
      ...
      do
      {
		chunk_nb_sector = (nb_sector >= NB_PAGE_SECTOR_PER_ERASE) ? NB_PAGE_SECTOR_PER_ERASE : nb_sector;
        EraseInitStruct.Sector = first_sector;
        EraseInitStruct.NbSectors = chunk_nb_sector;
        first_sector += chunk_nb_sector;
        nb_sector -= chunk_nb_sector;
  ...
  if (e_ret_status == HAL_OK)
  {
    ...
    else
    {
	  ...
	  for (i = 0U; i < uLength; i += 16U)
      {
		...
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, (uint32_t)pDestination,  ((uint32_t)(pdata+i)))== HAL_OK)
        {
