...
static void MX_DMA2D_Init(void)
{
  ...
  hdma2d.LayerCfg[1].InputAlpha = 128;
  ...
  hdma2d.LayerCfg[0].InputAlpha = 128;
...
static void MX_LTDC_Init(void)
{
  ...
  pLayerCfg.ImageWidth = STLogo_width;
  pLayerCfg.ImageHeight = STLogo_height;
...
void PerformScaling(DMA2D_DownscalingCfgTypeDef *cfg)
{
  ...
  if (HAL_DMA2D_BlendingStart_IT(&hdma2d, (uint32_t)STLogo, (uint32_t)LifeAugmented,
                                 LCD_FRAME_BUFFER, STLogo_width, STLogo_height) != HAL_OK)
  {
    Error_Handler();
  }
