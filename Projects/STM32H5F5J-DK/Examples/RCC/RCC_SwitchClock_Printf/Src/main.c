...
/**
  ******************************************************************************
  * @file    RCC/RCC_SwitchClock_Printf/Src/main.c
  ...
  *          and print a message to indicate each clock source switch.
  ******************************************************************************
  * @attention
  *
...
int main(void)
{
  ...
  /* Output SYSCLK  on MCO pin(PC.09) */
  HAL_RCC_MCOConfig(RCC_MCO2, RCC_MCO2SOURCE_SYSCLK, RCC_MCODIV_2);

  printf("Initial Clock Source: HSE\n");
...
static void SwitchSystemClock(void)
{
  if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSE)
  {
    ...
    /* Set SYSCLK frequency to 250 MHz, coming from the PLL which is clocked by CSI */
    SystemClockCSI_Config();
    printf("Clock Source Switched to CSI \n");
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_CSI)
  {
    /* PLL source is CSI oscillator */
    /* Set SYSCLK frequency to 250000000 Hz, coming from the PLL which is clocked by HSI  */
    SystemClockHSI_Config();
    printf("Clock Source Switched to HSI \n");
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSI)
  {
    /* PLL source is HSI oscillator */
    /* Set SYSCLK frequency to 250000000 Hz, coming from the PLL which is clocked by HSE  */
    SystemClockHSE_Config();
    printf("Clock Source Switched to HSE \n");
  }
...
/**
  * @brief  Switch the PLL source from HSE to CSI, and select the PLL as SYSCLK
  *         source.
...
static void SystemClockCSI_Config(void)
{
  ...
  */
  ...
  RCC_OscInitStruct.PLL.PLLR            = 2;
  RCC_OscInitStruct.PLL.PLLFRACN        = 0;
  RCC_OscInitStruct.PLL.PLLRGE          = RCC_PLLVCIRANGE_2;
...
/**
  * @brief  Switch the PLL source from CSI to HSI, and select the PLL as SYSCLK
  *         source.
  *         The system Clock is configured as follows :
