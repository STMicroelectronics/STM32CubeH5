...
The application creates 3 threads with different priorities :
  ...
  - usbx_app_thread_entry     (Priority : 10; Preemption threshold : 10) used to initialize USB DRD HAL HCD driver and start the Host.
...
When a hid device is plugged to STM32H553ZGTx board, a Message will be displayed on the UART HyperTerminal showing
...
- NUCLEO-H553ZG Set-up
    - Plug the USB HID device into the NUCLEO-H553ZG board through 'Type C  to A-Female' cable to the connector:
      - CN13 : to use USB Full Speed DRD IP (FS)
...
<b>Note</b>
 ...
 - The user must manually inject 5V onto the VBUS line by setting the hardware jumper to the 5V position.