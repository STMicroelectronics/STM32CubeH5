...
/* USER CODE BEGIN USBPD_PWR_IF_GetVBUSStatus */
  ...
  /* Parameters currently unused in this GPIO-based implementation */
  UNUSED(PortNum);
  UNUSED(PowerTypeStatus);

  /* Read the external VBUS detect GPIO and explicitly map its state
     to the USBPD return value convention */
  if (HAL_GPIO_ReadPin(USBC_DETECT_GPIO_Port, USBC_DETECT_Pin) == GPIO_PIN_SET)
  {
    return USBPD_TRUE;
  }
  else
