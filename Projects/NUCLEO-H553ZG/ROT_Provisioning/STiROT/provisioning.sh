...
data_xml="./Image/STiRoT_Data_Image.xml"
data_init_xml="./Image/STiRoT_Data_Init_Image.xml"
...
connect_boot0()
{
  echo "   * Please Press BOOT0 Button B3"
...
if [ "$isGeneratedByCubeMX" == "false" ]; then
  ...
  if [[ "$project_name" == "STiROT_Appli_TrustZone" ]]; then
    action="Update full secure to 0 in ${stirot_config}"
    $AppliCfg xmlval -v 0x00 --string -n "Is the firmware full secure" $stirot_config --vb >> $current_log_file
    if [ $? -ne 0 ]; then step_error; fi
  else
    action="Update full secure to 1 in ${stirot_config}"
    $AppliCfg xmlval -v 0x01 --string -n "Is the firmware full secure" $stirot_config --vb >> $current_log_file
    if [ $? -ne 0 ]; then step_error; fi
  fi
  action="Rebuild obk"
...
if [ "$isGeneratedByCubeMX" == "false" ]; then
  ...
  if [ "$full_secure" == "1" ]; then
    echo "       stm32h553xx_flash.icf and main.h successfully updated according to STiRoT_Config.xml"
  fi
  if [ "$full_secure" == "0" ]; then
    echo "       stm32h553xx_flash_s.icf and stm32h553xx_flash_ns.icf successfully updated according to STiRoT_Config.xml"
  fi
...
echo "   * BOOT0 Button B3 Should be Released"
echo "       Press any key to continue..."
echo
if [ "$mode" != "AUTO" ]; then read -p "" -n1 -s; fi
