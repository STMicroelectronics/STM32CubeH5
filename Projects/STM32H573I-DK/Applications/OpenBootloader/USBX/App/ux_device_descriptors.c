...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
  ...
  /* Set Option Bytes area 2 descriptor index in USBD_string_framework */
  count += len + 1;
  USBD_string_framework[count++] = USBD_LANGID_STRING & 0xFFU;
  USBD_string_framework[count++] = USBD_LANGID_STRING >> 8U;
  USBD_string_framework[count++] = USBD_DFU_STRING_OB2_DESC_INDEX;

  /* Set Option Bytes area 2 descriptor string in USBD_string_framework */
  USBD_Desc_GetString((uint8_t *)USBD_DFU_STRING_OB2_DESC, USBD_string_framework + count, &len);
