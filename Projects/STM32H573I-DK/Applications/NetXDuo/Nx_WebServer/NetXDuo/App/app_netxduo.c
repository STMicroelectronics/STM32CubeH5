...
static void LedThread_Entry(ULONG thread_input);
...
UINT MX_NetXDuo_Init(VOID *memory_ptr)
{
 ...
 /* Allocate the TCP server thread stack. */
  ...
  if(tx_byte_allocate(byte_pool, (VOID **) &pointer, SERVER_POOL_SIZE, TX_NO_WAIT) != TX_SUCCESS)
  {
    return TX_POOL_ERROR;
  }
  ...
  if(tx_byte_allocate(byte_pool, (VOID **) &pointer, SERVER_STACK, TX_NO_WAIT) != TX_SUCCESS)
  {
    return TX_POOL_ERROR;
  }
