...
p_thread_t _console_read_thd = 0 ;

/*===========================================================================*/
/* Service Functions                                                         */
/*===========================================================================*/

void UART_CallbackTypeDef(UART_HandleTypeDef *huart)
{
	if (_console_read_thd) {
		os_thread_notify_isr (&_console_read_thd, (int32_t)aRxBuffer[0]) ;
	}

	UART_Start_Receive_IT(&huart1, aRxBuffer, sizeof(aRxBuffer)) ;
}
...
console_service_run (uintptr_t arg)
{
    ...
    _console_read_thd = os_thread_current () ;
...
console_get_line (char * buffer, uint32_t len)
{
    ...
    while (!_shell_exit) {
    	int32_t c = os_thread_wait (400);
    	if (c == E_TIMEOUT) continue ;
    	__io_putchar((char)c) ;
        if (c == '\n') break;
        buffer[i++] = (char)c;
        if (i==len) break ;
