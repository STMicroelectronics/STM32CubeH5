...
static void MX_GPDMA2_Init(void)
{
  ...
  NVIC_EnableIRQ(GPDMA2_Channel0_IRQn);
...
static void MX_SPI1_Init(void)
{
  ...
  */
  ...
  LL_DMA_ConfigAddresses(GPDMA1, LL_DMA_CHANNEL_0,
                         (uint32_t)aTxBuffer, LL_SPI_DMA_GetTxRegAddr(SPI1));
...
static void MX_SPI3_Init(void)
{
  ...
  LL_RCC_SetSPIClockSource(LL_RCC_SPI3_CLKSOURCE_PLL1Q);
  ...
  */
  ...
  DMA_InitStruct.LinkedListBaseAddr = 0x00000000U;
  DMA_InitStruct.LinkedListAddrOffset = 0x00000000U;
  LL_DMA_Init(GPDMA2, LL_DMA_CHANNEL_0, &DMA_InitStruct);

  /* USER CODE BEGIN SPI3_Init 1 */
  /* Configure the GPDMA2_Channel0 functional parameters */
  LL_DMA_ConfigAddresses(GPDMA2, LL_DMA_CHANNEL_0,
                         LL_SPI_DMA_GetRxRegAddr(SPI3), (uint32_t)aRxBuffer);
  LL_DMA_SetBlkDataLength(GPDMA2, LL_DMA_CHANNEL_0, ubNbDataToTransmit);

  /* Enable DMA interrupts complete/error */
  LL_DMA_EnableIT_TC(GPDMA2, LL_DMA_CHANNEL_0);
  LL_DMA_EnableIT_DTE(GPDMA2, LL_DMA_CHANNEL_0);
  ...
  /* Set transfer size */
  LL_SPI_SetTransferSize(SPI3, ubNbDataToTransmit);
...
void WaitAndCheckEndOfTransfer(void)
{
  ...
  /* Disable GPDMA2 Rx Channel */
  LL_DMA_DisableChannel(GPDMA2, LL_DMA_CHANNEL_0);
...
/**
  * @brief  Function called from GPDMA2 IRQ Handler when Rx transfer is completed
  * @param  None
  * @retval None
  */
void GPDMA2_ReceiveComplete_Callback(void)
{
