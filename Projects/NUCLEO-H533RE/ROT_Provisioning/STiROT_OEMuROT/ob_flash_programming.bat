...
set remove_protect=-ob SECWM1_STRT=1 SECWM1_END=0 WRPSGn1=0xff WRPSGn2=0xff SECWM2_STRT=1 SECWM2_END=0 HDP1_STRT=1 HDP1_END=0 HDP2_STRT=1 HDP2_END=0 SECBOOT_LOCK=0xC3 SWAP_BANK=0 SRAM2_RST=0 SRAM2_ECC=0 BOOT_UBE=0xB4
set erase_all=-e all
...
IF !errorlevel! NEQ 0 goto :error
%stm32programmercli% %connect_no_reset% -ob SECWM2_STRT=0x1F SECWM2_END=0x0
IF !errorlevel! NEQ 0 goto :error
...
echo %action%
%stm32programmercli% %connect_no_reset% -d %cube_fw_path%\Projects\NUCLEO-H533RE\Applications\ROT\OEMiROT_Boot\Binary\rot_enc_sign.hex -v
IF !errorlevel! NEQ 0 goto :error
