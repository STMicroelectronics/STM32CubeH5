...
if [ "$isGeneratedByCubeMX" == "false" ]; then
  ...
  if [ "$full_secure" == "1" ]; then
    echo "       stm32h533xx_flash.icf and main.h successfully updated according to STiRoT_Config.xml"
  fi
  if [ "$full_secure" == "0" ]; then
    echo "       stm32h533xx_flash_s.icf and stm32h533xx_flash_ns.icf successfully updated according to STiRoT_Config.xml"
  fi
