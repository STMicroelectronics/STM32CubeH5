...
uint32_t SingleErrorData_Buffer[SRAM2_SIGNLE_ERROR_ADDRESS_SIZE];
uint32_t DoubleErrorData_Buffer[SRAM2_DOUBLE_ERROR_ADDRESS_SIZE];
/* USER CODE END PD */
...
int main(void)
{
  ...
  Fill_wMemory((uint32_t *)SRAM2_SIGNLE_ERROR_ADDRESS_BASE, (SRAM2_SIGNLE_ERROR_ADDRESS_SIZE * 2U), DATA_VALUE);
  ...
  Fill_wMemory((uint32_t *)SRAM2_SIGNLE_ERROR_ADDRESS_BASE, SRAM2_SIGNLE_ERROR_ADDRESS_SIZE, SINGLE_ERROR_DATA_VALUE);

  /* Fill memory with single error data */
  Fill_wMemory((uint32_t *)SRAM2_DOUBLE_ERROR_ADDRESS_BASE, SRAM2_DOUBLE_ERROR_ADDRESS_SIZE, DOUBLE_ERROR_DATA_VALUE);
  ...
  if (HAL_RAMCFG_RegisterCallback(&hramcfg_SRAM2, HAL_RAMCFG_SE_DETECT_CB_ID, SingleErrorDetectionCallback) != HAL_OK)
  {
    Error_Handler();
  }

  /* Register callback for RAMCFG peripheral double ECC error interrupt */
  if (HAL_RAMCFG_RegisterCallback(&hramcfg_SRAM2, HAL_RAMCFG_DE_DETECT_CB_ID, DoubleErrorDetectionCallback) != HAL_OK)
  {
    Error_Handler();
  }

  /* Enable RAMECC single and double ECC error interrupts */
  if (HAL_RAMCFG_EnableNotification(&hramcfg_SRAM2, (RAMCFG_IT_SINGLEERR | RAMCFG_IT_DOUBLEERR)) != HAL_OK)
  {
    Error_Handler();
  }

  /* Read single error memory area */
  Read_wMemory((uint32_t *)SRAM2_SIGNLE_ERROR_ADDRESS_BASE, SingleErrorData_Buffer, SRAM2_SIGNLE_ERROR_ADDRESS_SIZE);

  /* Read double error memory area */
  Read_wMemory((uint32_t *)SRAM2_DOUBLE_ERROR_ADDRESS_BASE, DoubleErrorData_Buffer, SRAM2_DOUBLE_ERROR_ADDRESS_SIZE);

  /* Check that all single error data are corrected via RAMCFG peripheral */
  if (Check_wBuffer(SingleErrorData_Buffer, SRAM2_SIGNLE_ERROR_ADDRESS_SIZE, DATA_VALUE) != MATCH)
  {
    Error_Handler();
  }

  /* Check that all double error data are not corrected via RAMCFG peripheral */
  if (Check_wBuffer(DoubleErrorData_Buffer, SRAM2_DOUBLE_ERROR_ADDRESS_SIZE, DOUBLE_ERROR_DATA_VALUE) != MATCH)
  {
    Error_Handler();
  }
  ...
  if (HAL_RAMCFG_DeInit(&hramcfg_SRAM2) != HAL_OK)
  {
    Error_Handler();
  }
