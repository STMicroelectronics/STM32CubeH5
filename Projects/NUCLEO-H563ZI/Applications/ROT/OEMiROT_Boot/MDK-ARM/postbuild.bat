...
set l_linker_file="%loader_dir%\MDK-ARM\stm32h563xx.sct"
...
set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m RE_IMAGE_FLASH_LOADER_OFFSET -n S_CODE_OFFSET %l_linker_file% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% definevalue --layout %preprocess_bl2_file% -m  RE_LOADER_S_CODE_SIZE -n S_CODE_SIZE %l_linker_file% --vb >> %current_log_file% 2>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
