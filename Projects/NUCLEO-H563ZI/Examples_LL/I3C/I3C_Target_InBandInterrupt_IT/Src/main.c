...
int main(void)
{
  ...
  /* Wait Dynamic Address completion */
  while(ubDynamicAddressCplt == 0U);
  ...
  while (1)
  {
    ...
    /*##- Send IBI request ###############################################*/
    /* Handle In Band Interrupt request */
    Handle_IBI_Target();
...
static void MX_I3C1_Init(void)
{
  ...
  */
  ...
  LL_I3C_SetDeviceIBIPayload(I3C1, LL_I3C_IBI_ADDITIONAL_DATA);
...
/**
  * @brief  This Function handle Target events to perform a IBI process
  * @note  This function is composed in one step :
  *        -1- Initiate the In Band Interrupt request.
  * @param  None
  * @retval None
  */
void Handle_IBI_Target(void)
{
  ...
  /* Target initiate a Start condition to inform Controller that Target request an asynchronous Interrupt.
  */
  /* Set IBI Payload */
  LL_I3C_SetIBIPayload(I3C1, ubPayloadBuffer[0]);

  /* Initiate IBI request */
  LL_I3C_TargetHandleMessage(I3C1, LL_I3C_TARGET_MTYPE_IBI, 1U);
}
...
void Target_Event_Treatment_Callback(void)
{
  ...
  if (LL_I3C_IsActiveFlag_DAUPD(I3C1))
  {
    ...
    /* Update Dynamic Address Completion Status */
    ubDynamicAddressCplt = 1;
  }

  /* Check IBI end process flag in EVR register */
  if (LL_I3C_IsActiveFlag_IBIEND(I3C1))
  {
    /* Clear IBI end process update flag */
    LL_I3C_ClearFlag_IBIEND(I3C1);

    /* Update IBI Completion Status */
    ubIBIcplt = 1;
  }
