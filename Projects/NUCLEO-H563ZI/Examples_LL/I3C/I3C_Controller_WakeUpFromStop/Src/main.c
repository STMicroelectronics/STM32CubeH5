...
void Handle_Repeat_Private_Controller(void);
...
int main(void)
{
  ...
  /*##- Start the Private communication to wakeup target ############################*/
  /* An auto stop is automatically generated after
  first element of aPrivateDescriptor or after a non acknowledge
  of first part of communication, due to wakeup latency on Target side */
  /* Handle Private transfer */
  Handle_Private_Controller();

  /* Wait end of Private transfer procedure */
  /* And manage repetition if a Non acknowledge is detected */
  do
  {
    if (ubNackDetected)
    {
      ...
      /* Handle the repetition of the previous Private transfer */
      Handle_Repeat_Private_Controller();
    }
  }
...
/**
  * @brief  This Function handle the repetition of a Private transfer to then from a target, when an error is detected
  * @note  This function is composed in one step :
  *        -1- Clear the different transfer counter.
  *        -2- Flush of the TX and Control FIFOs.
  *        -2- Call a new Private transfer.
  * @param  None
  * @retval None
  */
void Handle_Repeat_Private_Controller(void)
{
  uint32_t NbDataSent = ubNbTxData;

  /* Reset different communication counters */
  ubNbPrivate = 0;
  ubNbTxDataToTransfer += NbDataSent;
  ubNbTxData = 0;
  ...
  /* Flush Control FIFO */
  LL_I3C_RequestControlFIFOFlush(I3C1);
